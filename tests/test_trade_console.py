...
class TradeConsoleTests(TestCase):
    def test_format_trade_report_lists_actionable_brackets(self) -> None:
        ...
        self.assertIn("current state: held=0", output)
        self.assertIn("signals: 1 (C4=1)", output)
        self.assertIn("New bracket entries:", output)
        self.assertIn("PAM6", output)
        self.assertIn("0.3058", output)
        self.assertIn("0.2715", output)
        self.assertIn("30750", output)
    ...
    def test_format_trade_report_surfaces_state_skips_alerts_and_events(self) -> None:
        ...
        self.assertIn("Broker messages:", output)
        self.assertIn("2104", output)
        self.assertNotIn("BrokerErrorSeverity", output)
        self.assertIn("Maintenance / existing-position actions:", output)
